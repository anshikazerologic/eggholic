...
const FoodMenu = () => {
  ...
  // 2. Check Scroll Logic
  const checkScroll = () => {
    if (scrollRef.current) {
      const { scrollLeft, scrollWidth, clientWidth } = scrollRef.current;
      // Agar 2px se zyada scroll kiya hai tabhi left arrow dikhega
  ...
  }, [menuData, activeTab]); // Dependencies added
  ...
  // Get only the category names for the current tab
  const categoriesList = menuData[activeTab].map(catObj => catObj.category);

  // 2. Logic to render ONLY the items of the selected category
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    if (scrollRef.current) {
      scrollRef.current.scrollLeft = 0; // Scroll reset to start
    }
    if (menuData[tab] && menuData[tab].length > 0) {
      setActiveCategory(menuData[tab][0].category);
    }
    // Timeout taaki DOM update hone ke baad arrows check kare
  ...
  useEffect(() => {
    const handleScroll = throttle(() => {
      ...
      // 1. Sticky trigger: User has scrolled down 150px
      // This stays true even when we are at the footer
      setIsSticky(scrollPosition > 150);

      // 2. Bottom trigger: User is within 100px of the footer
      // This will trigger the opacity fade
