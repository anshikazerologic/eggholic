  ...
  catering: [
    {
      ...
      isVeg: true, // or false depending on choice
      description: "5 Sandwiches of your chosen flavor",
      image: "./Catering-4.png",
      items: [
        { id: 101, title: "Tray", description: "(Serves 5-6 People)", price: "$50" }
      ]
    },
    {
      category: "Dabeli Platter",
      isVeg: true,
      image: "./CATERING-DABELI.webp",
      items: [
        { id: 102, title: "Half Tray", description: "(9 Pieces)", price: "$35" },
        { id: 103, title: "Full Tray", description: "(18 Pieces)", price: "$65" }
      ]
    },
    {
      category: "Vada-Pav Platter",
      isVeg: true,
      image: "./vada pav tray-m.webp",
      items: [
        { id: 104, title: "Half Tray", description: "(9 Pieces)", price: "$40" },
        { id: 105, title: "Full Tray", description: "(18 Pieces)", price: "$75" }
      ]
    },
    {
      category: "Kathi Roll Platter",
      isVeg: false,
      description: "Your chosen flavor",
      image: "./Catering-6.png",
      items: [
        { id: 106, title: "Half Tray", description: "(8 Pieces)", price: "$45" },
        { id: 107, title: "Full Tray", description: "(16 Pieces)", price: "$90" }
      ]
    },
    {
      category: "Entrée Platters",
      isVeg: false,
      description: "Your chosen entrée (Egg/Veg or Chicken)",
      image: "./CATERING- ENTREE.webp",
      items: [
        { id: 108, title: "Half Tray (Egg/Veg)", description: "(Serves 8-10 People)", price: "$80" },
        { id: 109, title: "Full Tray (Egg/Veg)", description: "(Serves 16-18 People)", price: "$160" },
        { id: 110, title: "Half Tray (Chicken)", description: "(Serves 8-10 People)", price: "$100" },
        { id: 111, title: "Full Tray (Chicken)", description: "(Serves 16-18 People)", price: "$200" }
      ]
    }
  ]
};
...
const MobileMenu = () => {
  ...
  // Initialize with a generic default or handle dynamically
  ...
  // 1. FIX: Derive the data dynamically based on the active tab
  const currentCategories = MENU_DATA[activeTab];

  // Initialize activeCategory when tab changes
  ...
  // 2. UPDATED: Handle Tab Change
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    // Set active category to the first one in the new list
    const firstCategory = MENU_DATA[tab][0]?.category || "";
    setActiveCategory(firstCategory);

    // Reset scroll position of the category list
    if (categoryContainerRef.current) {
      categoryContainerRef.current.scrollLeft = 0;
    }
  };
  // 2. UPDATED: Category List based on current tab
  // Check if data exists before mapping to avoid errors
  const categoriesList = menuData[activeTab] ? menuData[activeTab].map(catObj => catObj.category) : [];

  // 3. UPDATED: Filtered Items logic
  const getFilteredItems = () => {
    const currentTabData = menuData[activeTab] || [];
    const selectedCatObj = currentTabData.find(cat => cat.category === activeCategory);

    if (selectedCatObj) {
      return selectedCatObj.items.map(item => ({
        ...item,
        isVeg: selectedCatObj.isVeg,
        // Ensure catering images use the category image if item image is missing
        image: item.image || selectedCatObj.image
      }));
    }
    return [];
  };
  ...
  return (
    <div className="mobile-app-container">
      ...
      <div className="cxz-toggle-container">
        <div className="cxz-toggle-wrapper2 shadow-sm">
          <div
            className={`cxz-toggle-item ${activeTab === 'online' ? 'cxz-active' : ''}`}
          ...
          <div
            className={`cxz-toggle-item ${activeTab === 'catering' ? 'cxz-active' : ''}`}
      ...
      <div className="mobile-category-outer">
        ...
        <div className="mobile-category-list" ref={categoryContainerRef}>
          {/* 3. FIX: Map through currentCategories instead of static 'data' */}
          {currentCategories.map((cat, index) => (
            <div
              ...
              onClick={() => {
                ...
                // Agar 'online' hai toh '/menu', warna '/catering' route use karein
                const routePath = activeTab === 'online' ? '/menu' : '/catering';

                navigate(`${routePath}?tab=${activeTab}&category=${encodeURIComponent(cat.category)}`);
              }}
            >
              <div className="mobile-category-img-box">
                <img src={cat.image || cat.items[0]?.image} alt={cat.category} />
              </div>
